...
export async function POST(request: NextRequest) {
    try {
        ...
        const raw = body?.clubId ?? body?.club_id;
        const clubIdStr =
            typeof raw === 'string'
                ? raw.trim()
                : raw && typeof raw === 'object' && '$oid' in raw
                  ? String((raw as { $oid: string }).$oid).trim()
                  : raw != null
                    ? String(raw).trim()
                    : '';

        if (!clubIdStr || !mongoose.Types.ObjectId.isValid(clubIdStr)) {
            return NextResponse.json(
                { success: false, message: '유효한 동아리 ID가 필요합니다' },
                { status: 400 }
            );
        }

        const clubOid = new mongoose.Types.ObjectId(clubIdStr);
        ...
        if (club.userId?.toString() === user._id.toString()) {
            return NextResponse.json(
                { success: false, message: '본인이 개설한 동아리입니다. 가입 신청이 필요하지 않습니다.' },
                { status: 400 }
            );
        }

        const chiefRecord = await ClubMember.findOne({
            userId: user._id,
            clubId: clubOid,
            role: 'chief',
        });
        if (chiefRecord) {
            return NextResponse.json(
                { success: false, message: '이 동아리의 회장으로 등록되어 있습니다. 가입 신청이 필요하지 않습니다.' },
                { status: 400 }
            );
        }
