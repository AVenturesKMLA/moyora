    ...
    try {
        ...
        return NextResponse.json({
            ...
            user: {
                ...
                clubRecommendPreference: user.clubRecommendPreference || 'popular',
...
const updateProfileSchema = z.object({
    ...
    clubRecommendPreference: z.enum(['popular', 'career']).optional(),
    careerInterest: z.string().max(500, '희망 진로는 500자 이내로 입력해주세요').optional(),
});
...
export async function PATCH(request: NextRequest) {
    try {
        ...
        const { name, schoolName, birthday, clubRecommendPreference, careerInterest } = validationResult.data;

        if (name) user.name = name;
        if (schoolName) user.schoolName = schoolName;
        if (birthday) user.birthday = new Date(birthday);
        if (clubRecommendPreference !== undefined) {
            user.clubRecommendPreference = clubRecommendPreference;
        }
        if (careerInterest !== undefined) {
            user.careerInterest = careerInterest.trim();
        }
