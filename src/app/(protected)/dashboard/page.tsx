...
function clubInitialsAvatarUrl(seed: string) {
    return `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(seed || '동아리')}&backgroundColor=0a0a0a`;
}

function RecentClubCard({ id, name, school, desc, intro, logoUrl }: { id: string, name: string, school: string, desc: string, intro?: string, logoUrl?: string }) {
    const themeLabel = desc === '체육' || desc === 'startup' ? '운동' : desc;
    const hoverText =
        (intro && intro.trim()) ||
        `${name} · ${school}`.trim();
    const hoverLabel = hoverText.length > 160 ? `${hoverText.slice(0, 157)}…` : hoverText;
    const avatarSrc = logoUrl && logoUrl.trim().length > 0 ? logoUrl.trim() : clubInitialsAvatarUrl(name);

    return (
        <div
            className="group relative flex items-center justify-between overflow-hidden rounded-xl border bg-card p-4 transition-colors hover:bg-accent/50"
            title={hoverLabel}
        >
            <div className="flex min-w-0 flex-1 items-center gap-3">
                ...
                <div className="min-w-0 space-y-1">
                    ...
                    <div className="mt-1 flex flex-wrap gap-1">
                        <Badge variant="outline" className="h-5 border-0 bg-muted/20 px-1.5 py-0 text-[10px] font-normal text-muted-foreground">
                            {themeLabel}
                        </Badge>
                    </div>
                </div>
            </div>
            <p className="pointer-events-none absolute inset-x-0 bottom-0 z-[1] line-clamp-3 bg-gradient-to-t from-card via-card/95 to-transparent px-4 pb-3 pt-10 text-xs leading-snug text-muted-foreground opacity-0 transition-all duration-200 group-hover:opacity-100 sm:px-4">
                {hoverLabel}
            </p>
            <div className="relative z-[2] flex shrink-0 items-center">
                <Button size="sm" variant="outline" className="h-8 rounded-full border-primary/40 px-3 text-xs text-primary hover:bg-primary hover:text-white" asChild>
